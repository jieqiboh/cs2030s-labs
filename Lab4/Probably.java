...
class Probably<T> implements Actionable<T>, Immutatorable<T>, Applicable<T> {
  ...
  public <R> Probably<R> transform(Immutator<R, T> immutator) {
    if (this.value == null) {
      return (Probably<R>) NONE;
    } else {
      return Probably.just(immutator.invoke(this.value));
    }
  }

  public Probably<T> check(Immutator<Boolean, T> i) {
    if (this.value == null) {
      return none();
    } else if (i.invoke(this.value)) {
      return this;
    } else {
      return none();
    }
  ...
  @Override
  public <R> Probably<R> apply(Probably<Immutator<R, T>> p) {
    if (!(p.value == null) && !(this.value == null)) {
