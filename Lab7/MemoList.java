import cs2030s.fp.Combiner;
import cs2030s.fp.Constant;
import cs2030s.fp.Immutator;
import cs2030s.fp.Memo;
import java.util.ArrayList;
import java.util.List;

/**
 * A wrapper around a lazily evaluated and memoized
 * list that can be generated with a lambda expression.
 *
 * @author Boh Jie Qi 
 * @version CS2030S AY 22/23 Sem 1
 */
class MemoList<T> {
  /** The wrapped java.util.List object */
  private List<Memo<T>> list;

  /** 
   * A private constructor to initialize the list to the given one. 
   *
   * @param list The given java.util.List to wrap around.
   */
  private MemoList(List<Memo<T>> list) {
    this.list = list;
  }

  /** 
   * Generate the content of the list.  Given x and a lambda f, 
   * generate the list of n elements as [x, f(x), f(f(x)), f(f(f(x))), 
   * ... ]
   *
   * @param <T> The type of the elements in the list.
   * @param n The number of elements.
   * @param seed The first element.
   * @param f The immutator function on the elements.
   * @return The created list.
   */
  public static <T> MemoList<T> generate(int n, T seed, Immutator<? extends T, ? super T> f) {
    MemoList<T> memoList = new MemoList<>(new ArrayList<>());
    Memo<T> curr = Memo.from(seed);

    for (int i = 0; i < n; i++) {
      memoList.list.add(curr);
      curr = curr.transform(f);
    }
    return memoList;
  }

  /**
   * Generate the content of the list using a combiner. Given x and a lambda f,
   * generate the list of n elements as 
   * [x, y, f(x, y), f(y, f(x, y)), f(f(x, y), f(y, f(x, y))), ... ]
   * ... ]
   *
   * @param <T> The type of the elements in the list.
   * @param n The number of elements.
   * @param fst The first element.
   * @param snd The second element.
   * @param f The combiner function on the elements.
   * @return The created list.
   */
  public static <T> MemoList<T> generate(int n, T fst, T snd,
                                         Combiner<? extends T, ? super T, ? super T> f) {
    MemoList<T> memoList = new MemoList<>(new ArrayList<>());
    Memo<T> memoFst = Memo.from(fst);
    memoList.list.add(memoFst);
    Memo<T> memoSnd = Memo.from(snd);
    memoList.list.add(memoSnd);

    Memo<T> prev = memoFst;
    Memo<T> curr = memoSnd;

    for (int i = 2; i < n; i++) {
      Memo<T> prevCapture = prev;
      Memo<T> currCapture = curr;

      prev = curr;
      curr = Memo.from(() -> f.combine(prevCapture.get(), currCapture.get()));

      memoList.list.add(curr);
    }
    return memoList;
  }

  /**
   * Generate a new list, with Immutator invoke applied to each element of the list.
   * @param <R> The type of elements in the list.
   * @param f the Immutator function on the elements.
   * @return The created list.
   */
  public <R> MemoList<R> map(Immutator<? extends R, ? super T> f) {
    MemoList<R> resMemoList = new MemoList<>(new ArrayList<>());

    this.list.forEach(m ->
            resMemoList.list.add(Memo.from(() -> f.invoke(m.get())))
    );
    return resMemoList;
  }

  /**
   * Generate a new list, taking an Immutator that takes each element, and returns a new
   * List, flattening the lists into a single list before returning it.
   * @param <R> The type of elements in the returned list.
   * @param f the Immutator function applied on the elements
   * @return The created list.
   */
  public <R> MemoList<R> flatMap(Immutator<MemoList<R>, ? super T> f) {
    MemoList<R> resMemoList = new MemoList<>(new ArrayList<Memo<R>>());

    for (int i = 0; i < this.list.size(); i++) {
      int pos = i;
      MemoList<R> subMemoList = f.invoke(this.get(pos));
      for (int j = 0; j < subMemoList.list.size(); j++) {
        int subPos = j;
        resMemoList.list.add(subMemoList.list.get(subPos));
      }
    }

    return resMemoList;
  }

  /** 
   * Return the element at index i of the list.  
   *
   * @param i The index of the element to retrieved (0 for the 1st element).
   * @return The element at index i.
   */
  public T get(int i) {
    return this.list.get(i).get();
  }

  /** 
   * Find the index of a given element.
   *
   * @param v The value of the element to look for.
   * @return The index of the element in the list.  -1 is element is not in the list.
   */
  public int indexOf(T v) {
    return this.list.indexOf(Memo.from(v));
  }

  /** 
   * Return the string representation of the list.
   *
   * @return The string representation of the list.
   */
  @Override
  public String toString() {
    return this.list.toString();
  }
}
